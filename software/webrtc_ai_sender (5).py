#!/usr/bin/env python3
"""
DriveEasy Eye - Advanced AI Smart Dashcam v4.1

SETUP:
  export GEMINI_API_KEY="your_key"
  python3 webrtc_ai_sender.py

v4.1 Changes (header upgrade only, nothing else changed):
  * Top-right of header now shows:
      Line 1: lat/lon in small text
      Line 2: City, State, India  (reverse geocoded via OSM Nominatim - FREE)
  * Reverse geocoding runs in background thread every 30 sec — never blocks video
  * Falls back to raw coords if no internet / GPS not fixed
  * Logo generated programmatically — no file needed
  * GPS writes location.json every 2 sec for web.py map
"""
...
def generate_logo(height=40):
    """Generates DriveEasy D logo programmatically. No file needed."""
    size  = height
...
# ==========================
# REVERSE GEOCODER (FREE - OSM Nominatim)
# ==========================

class ReverseGeocoder:
    """
    Converts GPS lat/lon to city, state, country using
    OpenStreetMap Nominatim API — completely FREE, no key needed.
    Runs in background thread every 30 seconds.
    Displays in header top-right:
      Line 1: lat, lon (small)
    ...
    def __init__(self):
        ...
        self._geocode_interval  = 30  # seconds between API calls
        print("Reverse geocoder ready (OpenStreetMap Nominatim - FREE)")

    def update_coords(self, lat, lon):
        """Call this whenever new GPS coords arrive."""
        with self._lock:
            self.lat = lat
            self.lon = lon

        # Only geocode every 30 seconds (don't hammer OSM)
        if time.time() - self._last_geocode_time > self._geocode_interval:
            self._last_geocode_time = time.time()
            threading.Thread(
                target=self._fetch_location,
                args=(lat, lon),
                daemon=True
            ).start()

    def _fetch_location(self, lat, lon):
        """Background fetch from OSM Nominatim."""
        try:
            url = (
            ...
            req.add_header("User-Agent", "DriveEasyEye/4.1 dashcam")
            ...
            # Shorten state name for header space
            state = state.replace("Telangana", "Telangana")
            state = state.replace("Andhra Pradesh", "A.P.")
            state = state.replace("Maharashtra", "Maharashtra")
            state = state.replace("Karnataka", "Karnataka")
            state = state.replace("Tamil Nadu", "T.N.")
            state = state.replace("Uttar Pradesh", "U.P.")
            state = state.replace("Madhya Pradesh", "M.P.")

            with self._lock:
                self.city    = city[:18]     # truncate long names
                self.state   = state[:16]
                self.country = "India"
            ...
            print(f"Geocode error (non-fatal): {e}")

    def get_header_lines(self):
        """
        Returns two strings for header display:
        ...
        parts = [p for p in [city, st, cntry] if p]
        line2 = ", ".join(parts) if parts else "Locating..."
...
class GPSReader:
    ...
    Writes location.json every 2 seconds for web.py map.
    Feeds coords to ReverseGeocoder automatically.
    """
    ...
    def __init__(self, geocoder=None):
        ...
        try:
            ...
            print("GPS not available — using placeholder location (Hyderabad)")
            # Set default Hyderabad coords so geocoder shows something
            self.lat = 17.3850
            self.lon = 78.4867
            if self._geocoder:
                self._geocoder.update_coords(17.3850, 78.4867)
            ...
            try:
                ...
                if report["class"] == "TPV":
                    ...
                    with self._lock:
                        ...
                        if lon is not None:
                            self.lon = lon
                    # Feed to reverse geocoder
                    if lat and lon and self._geocoder:
                        self._geocoder.update_coords(lat, lon)
...
class AIVideoStreamTrack(VideoStreamTrack):
    ...
    def __init__(self, camera, ai, gemini_ai, recorder, gps, geocoder):
        super().__init__()
        self.camera   = camera
        self.ai       = ai
    ...
    async def recv(self):
        ...
        # Get location lines from geocoder
        loc_line1, loc_line2 = self.geocoder.get_header_lines()

        # ── HEADER ────────────────────────────────────────────────────
        # Taller header to fit location info: 60px instead of 50px
        cv2.rectangle(frame, (0, 0), (fw, 60), (0, 0, 0), -1)

        # Logo (top-left, vertically centered in 60px header)
        overlay_logo(frame, self.logo, self.logo_mask, x_offset=5, y_offset=10)

        # Title text (left, next to logo)
        cv2.putText(frame, "DriveEasy Eye AI Dashcam",
                    (52, 28),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 255), 2)

        # ── LOCATION BLOCK (top-right of header) ──────────────────────
        # Line 1: lat, lon  — small, white/light
        # Measure text to right-align it
        (w1, _), _ = cv2.getTextSize(
            loc_line1, cv2.FONT_HERSHEY_SIMPLEX, 0.42, 1
        )
        ...
        # Line 2: City, State, India — slightly larger, cyan-ish
        if loc_line2:
            (w2, _), _ = cv2.getTextSize(
                loc_line2, cv2.FONT_HERSHEY_SIMPLEX, 0.46, 1
            )
        ...
        # Night badge (left of location block)
        if is_night:
            cv2.putText(frame, "[NIGHT]",
                        (fw - 340, 28),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 165, 255), 2)

        # REC indicator (just below night badge, or alone)
        if self.recorder.recording:
            dur = self.recorder.rec_duration()
            cv2.circle(frame, (fw - 340, 50), 8, (0, 0, 255), -1)
            cv2.putText(frame, f"REC {dur}",
                        (fw - 326, 55),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 0, 255), 2)

        # ── FOOTER (2-line, unchanged) ────────────────────────────────
...
async def run_webrtc_sender():
    ...
    gps      = GPSReader(geocoder=geocoder)
    ...
    async with websockets.connect(SIGNALING_SERVER) as ws:
        ...
        async for message in ws:
            ...
            if data["type"] == "request-offer":
                ...
                pc.addTrack(
                    AIVideoStreamTrack(
                        picam2, ai, gemini, recorder, gps, geocoder
                    )
                )

#!/usr/bin/env python3
"""
DriveEasy Eye - Advanced AI Smart Dashcam v4.2
...
v4.2 Fixes:
  * Header height fixed to 65px — location text fully visible
  * Default city = Hyderabad, Telangana, India (shows instantly on boot)
  * Reverse geocoder updates city when GPS connects (OSM Nominatim - FREE)
  * Location block right-aligned with padding — never goes off-screen
  * Logo, footer, all features unchanged
"""
...
HEADER_H         = 65        # header height in pixels
...
def generate_logo(height=42):
    """Generates DriveEasy D logo in code. No external file needed."""
    size  = height
    ...
    print(f"Logo generated: {size}x{size}px")
    return img, mask


def overlay_logo(frame, logo, mask, x_offset=6, y_offset=11):
    if logo is None:
        return
    lh, lw = logo.shape[:2]
    if (y_offset + lh) > frame.shape[0] or (x_offset + lw) > frame.shape[1]:
        return
...
class LocationManager:
    """
    Manages city/state display in header.

    - Default: Hyderabad, Telangana, India (shown instantly on boot)
    - Updates: when GPS connects, reverse-geocodes via OSM Nominatim (free)
    - Falls back gracefully if no internet

    Header top-right shows:
      Line 1:  17.3850, 78.4867          (small, light blue)
      Line 2:  Hyderabad, Telangana, India  (slightly larger, teal)
    """

    def __init__(self):
        # Default values shown immediately — no waiting
        self.lat     = 17.3850
        self.lon     = 78.4867
        self.city    = "Hyderabad"
        ...
        print("Location: default = Hyderabad, Telangana, India")

    def update_gps(self, lat, lon):
        """Call when new GPS coords arrive."""
        with self._lock:
            self.lat     = lat
            self.lon     = lon
            self.gps_fix = True

        # Reverse geocode every 30 seconds max
        now = time.time()
        if now - self._last_geocode > 30:
            self._last_geocode = now
            threading.Thread(
    ...
    def _geocode(self, lat, lon):
        """Background OSM Nominatim lookup — free, no API key."""
        try:
            url = (
            ...
            req.add_header("User-Agent", "DriveEasyEye/4.2 dashcam")
            ...
            # Shorten common long state names to fit header
            state = (state
                     .replace("Andhra Pradesh",  "A.P.")
                     .replace("Uttar Pradesh",   "U.P.")
                     .replace("Madhya Pradesh",  "M.P.")
                     .replace("Himachal Pradesh","H.P.")
            ...
            # Non-fatal — default Hyderabad still shows
            print(f"Geocode skipped: {e}")

    def get_header_lines(self):
        """
        Returns (line1, line2) strings for header.
        line1 = "17.3850, 78.4867"
        ...
        parts  = [p for p in [city, state, cntry] if p]
        line2  = ",  ".join(parts)
        return line1, line2
...
class GPSReader:
    """
    Reads from gpsd. Falls back silently if gpsd not running.
    Feeds coords into LocationManager for reverse geocoding.
    Writes location.json for web.py map.
    """

    def __init__(self, location_manager):
        self.loc_mgr   = location_manager
        self.speed_kmh = None
        self.enabled   = False
        self._lock     = threading.Lock()
        try:
            ...
            print("GPS not connected — default location: Hyderabad")
            self._write_json(17.3850, 78.4867, 0, False)
        ...
        while True:
            try:
                ...
                if report["class"] == "TPV":
                    ...
                    if lat and lon:
                        self.loc_mgr.update_gps(lat, lon)
    ...
    def _write_json(self, lat, lon, spd, valid):
        try:
...
class ManualRecorder:
    ...
    def start_http_server(self):
        ...
        print(f"Record server: http://0.0.0.0:{HTTP_PORT}/record")
...
class GeminiVisionAnalyzer:
    ...
    def _analyze(self, frame):
        ...
        prompt = (
            ...
            "warnings (max 3 items). Return [] if nothing notable. "
            "Example: [\"Road debris ahead\"] "
...
def detect_lane(frame, vehicles_present):
    ...
    cv2.fillPoly(mask, [np.array([[0, fh],[fw, fh],
                                  [fw, int(fh*0.65)],[0, int(fh*0.65)]], np.int32)], 255)
    raw = cv2.HoughLinesP(cv2.bitwise_and(edges, mask),
                          2, np.pi/180, 100, minLineLength=80, maxLineGap=40)
    if raw is None:
        return None
    filtered = []
    for ln in raw:
...
class AIVideoStreamTrack(VideoStreamTrack):
    ...
    def __init__(self, camera, ai, gemini_ai, recorder, gps, loc_mgr):
        super().__init__()
        self.camera   = camera
        self.ai       = ai
        ...
        # Logo generated once on startup
        self.logo, self.logo_mask = generate_logo(height=42)
    ...
    async def recv(self):
        ...
        # Location lines from manager
        loc_line1, loc_line2 = self.loc_mgr.get_header_lines()
        gps_fixed            = self.loc_mgr.is_fixed()

        # ── HEADER (65px tall black bar) ──────────────────────────────
        cv2.rectangle(frame, (0, 0), (fw, HEADER_H), (0, 0, 0), -1)

        # Logo — left side, vertically centred in header
        overlay_logo(frame, self.logo, self.logo_mask,
                     x_offset=6, y_offset=(HEADER_H - 42) // 2)

        # Title — next to logo, upper row
        cv2.putText(frame, "DriveEasy Eye AI Dashcam",
                    (56, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 255), 2)

        # ── LOCATION BLOCK — top-right of header ─────────────────────
        #
        #  |  17.3850,  78.4867           |  ← line1, small, light blue
        #  |  Hyderabad, Telangana, India  |  ← line2, teal-green
        #
        # GPS dot indicator: green = fix, gray = no fix
        dot_color = (0, 220, 80) if gps_fixed else (100, 100, 100)
        dot_x     = fw - 12
        cv2.circle(frame, (dot_x, 14), 5, dot_color, -1)

        # Line 1: lat/lon — right-aligned, 8px from right edge (before dot)
        (tw1, th1), _ = cv2.getTextSize(
            loc_line1, cv2.FONT_HERSHEY_SIMPLEX, 0.40, 1
        )
        cv2.putText(frame, loc_line1,
                    (fw - tw1 - 22, 18),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.40,
                    (160, 210, 255), 1)

        # Line 2: city, state, India — right-aligned
        (tw2, th2), _ = cv2.getTextSize(
            loc_line2, cv2.FONT_HERSHEY_SIMPLEX, 0.44, 1
        )
        cv2.putText(frame, loc_line2,
                    (fw - tw2 - 22, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.44,
                    (0, 210, 160), 1)

        # Line 3: "GPS FIX" or "DEFAULT LOC" label — very small, muted
        fix_label = "GPS FIX" if gps_fixed else "DEFAULT LOC"
        (tw3, _), _ = cv2.getTextSize(
            fix_label, cv2.FONT_HERSHEY_SIMPLEX, 0.32, 1
        )
        cv2.putText(frame, fix_label,
                    (fw - tw3 - 22, 57),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.32,
                    (0, 150, 100) if gps_fixed else (80, 80, 100), 1)
        ...
        if self.recorder.recording:
            ...
            cv2.circle(frame, (260, 52), 7, (0, 0, 255), -1)
        ...
        # Detection telemetry (centre bottom)
...
async def run_webrtc_sender():
    ...
    loc_mgr  = LocationManager()          # default = Hyderabad instantly
    gps      = GPSReader(loc_mgr)         # updates when GPS connected
    ...
    async with websockets.connect(SIGNALING_SERVER) as ws:
        ...
        async for message in ws:
            ...
            if data["type"] == "request-offer":
                ...
                pc.addTrack(
                    AIVideoStreamTrack(
                        picam2, ai, gemini, recorder, gps, loc_mgr
                    )
                )

#!/usr/bin/env python3
"""
DriveEasy Eye - Advanced AI Smart Dashcam v3.1

SETUP:
  1. Copy logo:   cp logo.png /home/driveeasy/ai_dashcam/logo.png
  2. Set API key: export GEMINI_API_KEY="your_key"
  3. Run:         python3 webrtc_ai_sender.py

All previous features kept exactly as-is.

v3.1 Changes only:
  * DriveEasy logo shown in header (left of title text)
  * Lane lines HARD FIXED - angle filter removes vertical wall/pillar lines
    (only 20-65 and 115-160 degree lines kept = true road markings only)
  * Lane lines minimum length raised to 80px
"""
...
def load_logo(path, height=40):
    if not os.path.exists(path):
        print(f"Logo not found at {path} - running without logo.")
        return None, None
    ...
    print(f"Logo loaded: {new_w}x{height}px")
    return resized, mask


def overlay_logo(frame, logo, mask, x_offset=10, y_offset=5):
    if logo is None:
        return
    lh, lw = logo.shape[:2]
...
class GPSReader:
    def __init__(self):
        ...
        try:
            ...
            self.gpslib  = gpslib
            self.session = gpslib.gps(mode=gpslib.WATCH_ENABLE | gpslib.WATCH_NEWSTYLE)
            self.enabled = True
            threading.Thread(target=self._loop, daemon=True).start()
            print("GPS reader started")
        except Exception:
            print("GPS not available - showing --")
...
class ManualRecorder:
    """
    HTTP toggle server.
    http://PI_IP:8080/record  -> start/stop
    http://PI_IP:8080/status  -> JSON status
    """
    ...
    def start_http_server(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self):
                if self.path == "/record":
                    ...
                    self.end_headers()
                    self.wfile.write(json.dumps({"recording": recorder.recording}).encode())
                else:
        ...
        print(f"Manual record server  ->  http://0.0.0.0:{HTTP_PORT}/record")
    ...
    def toggle(self):
        with self._lock:
            if not self.recording:
                ...
                self._frames   = []
                threading.Thread(target=self._write, args=(frames,), daemon=True).start()
    ...
    def _write(self, frames):
        if not frames:
            return
        path = os.path.join(EVENT_FOLDER, datetime.now().strftime("manual_%Y%m%d_%H%M%S.avi"))
...
class DistanceEstimator:
    def __init__(self, focal_length=FOCAL_LENGTH):
...
class NightModeEnhancer:
    ...
    def process(self, frame):
        self.night_mode = np.mean(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)) < self.threshold
        if self.night_mode:
...
class FatigueMonitor:
    ...
    def check(self):
        ...
        if elapsed >= self.warn_after and time.time() - self.last_warn >= self.remind:
...
class EventVideoBuffer:
    ...
    def save(self, tag="event"):
        ...
        self._saving = True
        threading.Thread(target=self._write, args=(frames, tag), daemon=True).start()

    def _write(self, frames, tag):
        if not frames:
            self._saving = False
            return
        path = os.path.join(EVENT_FOLDER, datetime.now().strftime(f"{tag}_%Y%m%d_%H%M%S.avi"))
        h, w = frames[0].shape[:2]
        out  = cv2.VideoWriter(path, cv2.VideoWriter_fourcc(*"XVID"), self.fps, (w, h))
        for f in frames:
            out.write(f)
...
class GeminiVisionAnalyzer:
    ...
    def _analyze(self, frame):
        ...
        resp  = self.model.generate_content([{"mime_type": "image/jpeg", "data": buf.tobytes()}, prompt])
        match = re.search(r'\[.*?\]', resp.text.strip(), re.DOTALL)
        with self._lock:
            self.warns = [str(w) for w in json.loads(match.group())[:3]] if match else []
...
# ==========================
# LANE DETECTION (v3.1 HARD FIXED)
# ==========================

def detect_lane(frame, vehicles_present):
    """
    v3.1 fixes:
    - Only runs when vehicles detected ahead (no false indoor triggers)
    - ROI = bottom 35% only (road zone)
    - Angle filter: EXCLUDES 70-110 degree range (near-vertical lines)
      This removes wall edges, pillars, door frames completely.
      Only near-horizontal road markings pass through.
    - Min line length raised to 80px
    """
    ...
    for line in raw_lines:
        ...
        # Exclude near-vertical lines (70-110 degrees = walls, pillars, edges)
        if not (70 <= angle <= 110):
...
# ==========================
# COLLISION WARNING (vehicles only)
# ==========================
...
class AIVideoStreamTrack(VideoStreamTrack):
    ...
    def __init__(self, camera, ai, gemini_ai, recorder):
        super().__init__()
        self.camera   = camera
        self.ai       = ai
        ...
        self.logo, self.logo_mask = load_logo(LOGO_PATH, height=40)
    ...
    async def recv(self):
        ...
        logo_w = self.logo.shape[1] if self.logo is not None else 0
        overlay_logo(frame, self.logo, self.logo_mask, x_offset=5, y_offset=5)

        # Title shifted right of logo
        title_x = logo_w + 15 if self.logo is not None else 20
        ...
        telemetry = f"Persons:{persons}  Vehicles:{vehicles}  Traffic:{traffic}  Dist:{dist_str}"
        if signal_color:
...
async def run_webrtc_sender():
    ...
    async with websockets.connect(SIGNALING_SERVER) as ws:
        ...
        await ws.send(json.dumps({"type": "register-pi", "deviceId": DEVICE_ID}))
        ...
        async for message in ws:
            ...
            if data["type"] == "request-offer":
                ...
                pc.addTrack(AIVideoStreamTrack(picam2, ai, gemini, recorder))
